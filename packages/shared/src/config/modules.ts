...
export const getDefaultModuleInfo = (
	name: string,
	path: string,
): ModuleInfo => ({
	name: name,
	...
	endpointPathBuilder: (version: string = MODULES_VERSION) =>
		`modules/${version}/${path}.js`,
	devServerPath: `/../dist/modules/${MODULES_VERSION}/${path}.js`,
});

export const epic: ModuleInfo = getDefaultModuleInfo(
	'epic',
	'epics/ContributionsEpic',
);
