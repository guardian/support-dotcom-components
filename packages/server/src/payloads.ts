...
import {
	addTickerDataToSettings,
	getTickerSettings,
} from './lib/fetchTickerData';
import {
...
import {
	...
	findTestAndVariant,
} from './tests/epics/epicSelection';
...
export const buildEpicData = async (
	...
	const superModeArticles = enableSuperMode
		? await fetchSuperModeArticlesCached()
		: [];
	...
	if (process.env.log_targeting === 'true') {
		console.log(
			`Renders Epic ${
				result ? 'true' : 'false'
			} for targeting: ${JSON.stringify(targeting)}`,
		);
	}
...
export const buildHeaderData = async (
	...
	if (testSelection) {
		...
		return {
			data: {
				module: {
					url: `${baseUrl}/${variant.modulePathBuilder(
						targeting.modulesVersion,
					)}`,
					name: header.name,
					props: {
