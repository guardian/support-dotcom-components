...
const fetchAmpEpicTests = (): Promise<AmpEpicTest[]> =>
	fetchS3Data(
		...
		`epic/${isProd ? 'PROD' : 'CODE'}/amp-epic-tests.json`,
	)
		.then(JSON.parse)
		.then((data) => {
			return data.tests;
...
export const selectAmpEpicTestAndVariant = async (
	...
	const test = tests.find(
		(test) => test.isOn && inCountryGroups(countryCode, test.locations),
	);
	...
	if (test && test.variants) {
		...
		const variant = test.variants.find(
			(variant) =>
				...
				assignedVariantName.toUpperCase(),
		);
		...
		if (variant) {
			...
			const epicData = {
				...
				heading: replaceNonArticleCountPlaceholders(
					variant.heading,
					countryCode,
				),
