...
export const BannerPaths: {
	...
	[BannerTemplate.ContributionsBanner]:
		contributionsBanner.endpointPathBuilder,
...
const createTestsGeneratorForChannel = (
				...
				return tests.map((testParams: RawTestParams): BannerTest => {
					return {
						...
						articlesViewedSettings:
							testParams.articlesViewedSettings,
						variants: testParams.variants.map(
							BannerVariantFromParams,
						),
						controlProportionSettings:
							testParams.controlProportionSettings,
					};
				});
			});
};

export const channel1BannersAllTestsGenerator =
	createTestsGeneratorForChannel('contributions');
export const channel2BannersAllTestsGenerator =
	createTestsGeneratorForChannel('subscriptions');
