...
const getDesignForVariant = (variant: BannerVariantFromTool): ConfigurableDesign | undefined => {
    if (uiIsDesign(variant.template)) {
        // TODO: actually look up the design by name and expand
        return {
            image: {
                mobileUrl:
...
const buildBannerVariant = (forChannel: BannerChannel) => (
    ...
    design: getDesignForVariant(variant),
});
