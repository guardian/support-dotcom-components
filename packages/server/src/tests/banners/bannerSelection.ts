...
import type {
	BannerDeployCaches,
	ReaderRevenueRegion,
} from './bannerDeployCache';

export const readerRevenueRegionFromCountryCode = (
	countryCode: string,
): ReaderRevenueRegion => {
	switch (true) {
		case countryCode === 'GB':
...
export const redeployedSinceLastClosed = (
	...
	const { subscriptionBannerLastClosedAt, engagementBannerLastClosedAt } =
		targeting;
...
const getForcedVariant = (
	...
	const test = tests.find(
		(test) =>
			...
			forcedTestVariant.testName.toLowerCase(),
	);
	const variant = test?.variants.find(
		(v) =>
			v.name.toLowerCase() ===
			forcedTestVariant.variantName.toLowerCase(),
	);
	...
	if (test && variant) {
		return {
			test,
			variant,
			moduleUrl: `${baseUrl}/${variant.modulePathBuilder(
	...
	for (const test of tests) {
		if (
			...
			!(
				...
				targeting.hasOptedOutOfArticleCount
			) &&
