		...
		try {
			...
			res.locals.bannerTargeting = {
				...
				engagementBannerLastClosedAt:
					targeting.engagementBannerLastClosedAt,
				subscriptionBannerLastClosedAt:
					targeting.subscriptionBannerLastClosedAt,
...
const createEndpointForModule = (moduleInfo: ModuleInfo): void => {
	app.get(
		...
		async (
			...
			try {
				const module = await fs.promises.readFile(
					__dirname + moduleInfo.devServerPath,
				);
...
app.get(
	...
	async (
		...
		try {
			...
			const campaignCode = buildAmpEpicCampaignCode(
				epic.testName,
				epic.variantName,
			);
...
app.post('/puzzles', async (req: express.Request, res: express.Response) => {
	const { tracking, targeting } = req.body;
	const response = await buildPuzzlesData(
		tracking,
		targeting,
		req.params,
		req,
