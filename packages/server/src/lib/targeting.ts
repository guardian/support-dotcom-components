...
export const shouldThrottle = (
	...
	const hasReachedViewsLimitInWindow =
		viewsInThrottleWindow.length >= config.maxViewsCount;

	const withinMinDaysSinceLastView = viewsInThrottleWindow.some(
		(view) =>
			daysSince(new Date(view.date), now) < config.minDaysBetweenViews,
	);

	return hasReachedViewsLimitInWindow || withinMinDaysSinceLastView;
};

export const shouldNotRenderEpic = (
	meta: EpicTargeting,
	epicType: EpicType,
): boolean => {
	const isLowValueSection = lowValueSections.some(
		(id) => id === meta.sectionName,
	);
	const isLowValueTag = lowValueTags.some((id) =>
		meta.tags.some((pageTag) => pageTag.id === id),
	);
