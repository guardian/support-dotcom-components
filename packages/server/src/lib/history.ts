...
export const getArticleViewCountForWeeks = (
	...
	const weeksInWindow = history.filter(
		(weeklyArticleLog: WeeklyArticleLog) =>
			weeklyArticleLog.week >= cutOffWeek,
	);

	return weeksInWindow.reduce(
		(accumulator: number, currentValue: WeeklyArticleLog) =>
			currentValue.count + accumulator,
		0,
	);
};
...
export const historyWithinArticlesViewedSettings = (
	...
	const viewCountForWeeks = getArticleViewCountForWeeks(
		history,
		periodInWeeks,
		now,
	);
