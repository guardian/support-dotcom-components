import React from 'react';
import { ImageSize } from './headerImageEnvironment';

interface HeaderImageSvgProps {
    imageSize: ImageSize;
}

export function HeaderImageSvgEnvironment({ imageSize }: HeaderImageSvgProps): JSX.Element {
    const viewBox = '0 0 ' + imageSize.width + ' ' + imageSize.height;
    return (
        <svg
            width={imageSize.width}
            height={imageSize.height}
            viewBox={viewBox}
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M61.3482 35.8789L85 35.9999L83.4819 26.7649L58.205 24.8996L61.3482 35.8789Z"
                fill="#F50000"
            />
            <path
                d="M41.355 25.1731L9.99956 25.8901L11.6444 15.445L32.2081 16.0838L41.355 25.1731Z"
                fill="#FFCB00"
            />
            <path
                d="M93.1629 10.514L126.397 8.60674L122.06 18.4909L93.0002 17.9595L93.1629 10.514Z"
                fill="black"
            />
            <path
                d="M46.7076 13.3267L69.9996 13L73.1169 18.4726L43.0494 20.3337L46.7076 13.3267Z"
                fill="#24ADC8"
            />
            <path
                d="M301.3 18.0615L278.312 19.3031L274 15.7579L300.537 15.3102L301.3 18.0615Z"
                fill="#4DA65C"
            />
            <path
                d="M504.3 28.7544L481.312 29.996L477 26.4508L503.537 26.0031L504.3 28.7544Z"
                fill="#4DA65C"
            />
            <path
                d="M340.841 51.0659L306.957 53.5231L300.164 41.4514L339.392 41.8842L340.841 51.0659Z"
                fill="#4DA65C"
            />
            <path
                d="M10.3519 18.27L22.0983 18.5303L21.472 12.4613L0.276392 11.992L10.3519 18.27Z"
                fill="#4DA65C"
            />
            <path
                d="M347.065 35.9795L316.755 35.9973L311 29L347.065 26.899L347.065 35.9795Z"
                fill="#4DA65C"
            />
            <path
                d="M279.669 25.6465L280.942 30.2622L250.141 31.258L243 26.5682L279.669 25.6465Z"
                fill="#F50000"
            />
            <path
                d="M489.701 7.43303L491.222 18.1447L460.405 18.7691L453 7.56821L489.701 7.43303Z"
                fill="#F50000"
            />
            <path
                d="M638.091 25.9061L639.075 33.3473L619.745 33.6948L615.069 25.8976L638.091 25.9061Z"
                fill="#F50000"
            />
            <path
                d="M128.558 22.2268L103 22.1594L104.634 28.076L131.947 29.2605L128.558 22.2268Z"
                fill="#4DA65C"
            />
            <path d="M346 13L310 12.0003L313.548 5.02217L355.5 3.49992L346 13Z" fill="#0053D8" />
            <path
                d="M366 28.0298L402.707 28.6086L403.895 36.5094L371.121 36.5353L366 28.0298Z"
                fill="#F50000"
            />
            <path
                d="M546.895 10.0298L510.187 10.6086L509 18.5094L541.774 18.5353L546.895 10.0298Z"
                fill="#0053D8"
            />
            <path
                d="M208.305 21.3413L169.011 23.6419L178 13.5L208.297 12.0002L208.305 21.3413Z"
                fill="#24ADC8"
            />
            <path
                d="M445.305 24.3413L406.011 26.6419L410.649 15.8487L445.297 15.0002L445.305 24.3413Z"
                fill="#24ADC8"
            />
            <path
                d="M149.22 24.5843L126 25.4913L126.265 19.2489L152.431 17.1857L149.22 24.5843Z"
                fill="#FFCB00"
            />
            <path
                d="M376.22 15.6457L353 14.7386L353.265 20.9811L379.431 23.0443L376.22 15.6457Z"
                fill="#FFCB00"
            />
            <path
                d="M326.14 29.4392L302.92 30.1148L303.185 25.4654L329.351 23.9287L326.14 29.4392Z"
                fill="#FFCB00"
            />
            <path
                d="M252.39 16.0025L234.911 16.8757L235.635 11.4016L254.044 11.0225L252.39 16.0025Z"
                fill="#FFCB00"
            />
            <path
                d="M218.5 19.4085L243.997 19.4085L243.049 24.2344L216.348 24.7652L218.5 19.4085Z"
                fill="#0053D8"
            />
            <path
                d="M176.068 32.4849L159.963 33.2898L159.957 27.005L178.181 28.0469L176.068 32.4849Z"
                fill="black"
            />
            <path
                d="M418.725 9.89941L402.62 9.0945L402.614 15.3792L420.838 14.3374L418.725 9.89941Z"
                fill="black"
            />
            <path
                d="M600.997 29L576.5 29.9999L576.5 34.4995L594.5 34.4995L600.997 29Z"
                fill="black"
            />
            <path
                d="M562.49 20.0835L623.216 20.2852L613.526 10L562.49 10.605V20.0835Z"
                fill="#4AA757"
            />
            <path
                d="M553.721 19.293L584.844 18.9993L584.289 30.7605L550.15 33.7956L553.721 19.293Z"
                fill="#FCCC08"
            />
        </svg>
    );
}
