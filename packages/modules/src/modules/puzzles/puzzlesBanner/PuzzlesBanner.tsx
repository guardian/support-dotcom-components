...
export const PuzzlesBanner: React.FC<PuzzlesBannerProps> = ({
	tracking,
	...
	function minimise() {
		const stateChange: BannerStateChange = isMinimised
			? 'expand'
			: 'minimise';
	...
	const MinimiseButton = (
		<Button
			...
			icon={
				isMinimised ? <SvgArrowUpStraight /> : <SvgArrowDownStraight />
			}
			onClick={minimise}
			hideLabel
		>
	...
	useEffect(() => {
		const bannerLoadState: BannerState = isMinimised
			? 'minimised'
			: 'expanded';
