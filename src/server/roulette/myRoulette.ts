...
export function myRoulette<V extends Variant, T extends Test<V>>(
    ...
    const sumOfMeans = testBanditData.bestVariants.reduce((sum, v) => sum + v.mean, 0);

    // sorted variant weights, which will add up to 1
    const variantsWithWeights: { weight: number; variantName: string }[] =
        testBanditData.bestVariants
            .map(({ variantName, mean }) => ({
                variantName,
                weight: mean / sumOfMeans,
            }))
